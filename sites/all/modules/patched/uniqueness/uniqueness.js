...
(function($) {
    var title_field, uniqueness;
    Drupal.behaviors.uniqueness = {
        attach: function (context) {
            $("#edit-uniqueness").hide();
            if (!uniqueness) {
                uniqueness = new Drupal.uniqueness(Drupal.settings.uniqueness['URL'], $('.uniqueness-dyn'));
            }
            // Check for title module.
            if (document.getElementById('edit-title-field-und-0-value') != null) {
                title_field = '#edit-title-field-und-0-value';
            }
            else {
                title_field = '#edit-title';
            }
            // Search off title.
            $(title_field, context).once('uniqueness', function() {
                $(this).keyup(function() {
                    input = this.value;
                    if (input.length >= uniqueness.minCharacters) {
                        uniqueness.search('title', input);
                        $("#edit-uniqueness").show();
                    }
    ...
    Drupal.uniqueness.prototype.update = function (data) {
        ...
        if (uniqueness.prependResults) {
            ...
            if (data == undefined) {
                uniqueness.clear();
                if ($(title_field)[0].value.length >= uniqueness.minCharacters) {
